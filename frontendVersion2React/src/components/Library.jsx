...
function Library({ isLoggedIn, onPlaylistUpdate }) {
  ...
  const handleCreatePlaylist = async () => {
    ...
    try {
      ...
      await apiFetch('/playlists', 'POST', newPlaylistData);
  ...
  const createIconSVG = (
    <svg width="22" height="21" viewBox="0 0 24 23" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M12.2841 -0.00683594V23.0001" stroke="#626262" strokeWidth="3" />
      <path d="M0 11.6562H24" stroke="#626262" strokeWidth="3" />
    </svg>
  );
      ...
      <div id="sidebar-library-content" className={isLoggedIn ? '' : 'hidden'}>
        <div className="library-header">
          ...
          <button
            ...
            className={`btn-create btn-icon ${isCreating ? 'is-loading' : ''}`}
            disabled={isCreating}
          >
            {createIconSVG}
            Crear lista
